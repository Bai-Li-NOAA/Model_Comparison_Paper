...
# sel_survey$survey1$A50.sel1 <- 4
# sel_survey$survey1$slope.sel1 <- 1
# sel_survey$survey1$slope.sel2 <- 0.7
# sel_survey$survey1$A50.sel2 <- 11
...
#sel_survey$survey2$slope.sel1 <- 1.5
...
mat.age=logistic(pattern=1, x=ages, a1=slope.mat, b1=A50.mat) #maturity at age

selex_fleet <- vector(mode="list", length=fleet_num)
names(selex_fleet) <- paste("fleet", 1:fleet_num, sep="")
invisible(sapply(1:length(selex_fleet), function(x) selex_fleet[[x]] <<- logistic(pattern=2, x=ages, a1=sel_fleet[[x]]$slope.sel1, b1=sel_fleet[[x]]$A50.sel1, a2=sel_fleet[[x]]$slope.sel2, b2=sel_fleet[[x]]$A50.sel2)))

selex_survey <- vector(mode="list", length=survey_num)
names(selex_survey) <- paste("survey", 1:survey_num, sep="")
#invisible(sapply(1:length(selex_survey), function(x) selex_survey[[x]] <<- logistic(pattern=1, x=ages, a1=sel_survey[[x]]$slope.sel1, b1=sel_survey[[x]]$A50.sel1)))
invisible(sapply(1:length(selex_survey), function(x) selex_survey[[x]] <<- logistic(pattern=2, x=ages, a1=sel_survey[[x]]$slope.sel1, b1=sel_survey[[x]]$A50.sel1, a2=sel_survey[[x]]$slope.sel2, b2=sel_survey[[x]]$A50.sel2)))
