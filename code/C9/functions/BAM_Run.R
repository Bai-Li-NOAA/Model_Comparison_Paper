BAM_Run <- function(maindir=maindir, subdir="BAM", om_sim_num=1){
  ...
  for (om_sim in 1:om_sim_num){
    ...
    if (modify_input == "partial"){
      ...
      dat1.survey1=list(nyr=length(em_input$survey.obs$survey1), yrs=om_input$yr, vals.obs=em_input$survey.obs$survey1, cv=rep(em_input$cv.survey$survey1, length(em_input$survey.obs$survey1)), nyr.ages=length(em_input$survey.obs$survey1), yrs.age=om_input$yr, nsamp=rep(em_input$n.survey$survey1, length(em_input$survey.obs$survey1)), nfish=rep(em_input$n.survey$survey1, length(em_input$survey.obs$survey1)), acomp=em_input$survey.age.obs$survey1)

      dat1.survey2=list(nyr=length(em_input$survey.obs$survey2), yrs=om_input$yr, vals.obs=em_input$survey.obs$survey2, cv=rep(em_input$cv.survey$survey2, length(em_input$survey.obs$survey2)), nyr.ages=length(em_input$survey.obs$survey2), yrs.age=om_input$yr, nsamp=rep(em_input$n.survey$survey2, length(em_input$survey.obs$survey2)), nfish=rep(em_input$n.survey$survey2, length(em_input$survey.obs$survey2)), acomp=em_input$survey.age.obs$survey2)
      ...
      BAM.write.dat(fname=file.path(maindir, "output", subdir, paste("s", om_sim, sep=""), "BAM-Sim.dat"), nyr=om_input$nyr, nages=om_input$nages, dat.survey1=dat1.survey1, dat.survey2=dat1.survey2, dat.L=dat1.L, parms=bam_parms, a.lw=om_input$a.lw, b.lw=om_input$b.lw, prop.f=om_input$proportion.female, om_input$mat.age,om_input$M.age)
    }
