  ...
  for (om_sim in 1:om_sim_num){
    ...
    if(modify_input == "partial") {
      ...
      ss_data$CPUE <- as.data.frame(rbind(
        cbind(om_input$yr, rep(1, times=om_input$nyr), rep(2, times=om_input$nyr), em_input$survey.obs$survey1, rep(sqrt(log(1+em_input$cv.survey$survey1^2)), times=length(em_input$survey.obs$survey1))),
        cbind(om_input$yr, rep(1, times=om_input$nyr), rep(3, times=om_input$nyr), em_input$survey.obs$survey2, rep(sqrt(log(1+em_input$cv.survey$survey2^2)), times=length(em_input$survey.obs$survey2)))))
