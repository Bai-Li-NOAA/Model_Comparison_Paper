...
msy_sim_boxplot <-boxplot(msy_re[,seq(dim(msy_re)[2],1)], horizontal=T, col=col[5:2], pch=16, cex=0.5, axes=F, at=c(1,2,3,4, 6,7,8,9, 11,12,13,14), xlab="Relative Error", ylim=c(-0.5,0.5), outline=F)
...
temp_boxplot <- boxplot(temp[,2:ncol(temp)], beside=T, axes=F, ylab=parameters[1], col=col, pch=16, ylim=range(temp), outline=F)
box()
...
temp_boxplot <- boxplot(temp[,2:ncol(temp)], beside=T, axes=F, ylab=parameters[2], col=col, pch=16, outline=F)
box()
axis(1, at=1:4, labels = c("AMAK", "ASAP", "BAM", "SS"))
axis(2)
...
for (i in 1:length(plot_data_id)){
  ...
  boxplot(amak_list[[plot_data_id[i]]][om_input$nyr,], asap_list[[plot_data_id[i]]][om_input$nyr,], bam_list[[plot_data_id[i]]][om_input$nyr,], ss_list[[plot_data_id[i]]][om_input$nyr,], col=col, axes=F, ylab=rownames(plot_data)[i], pch=16, outline=F)
  box()
  axis(1, at=1:4, labels = c("AMAK", "ASAP", "BAM", "SS"))
  axis(2)
