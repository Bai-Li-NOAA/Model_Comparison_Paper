...
for(om_sim in 1:om_sim_num){
  ...
  R0=convertSRparms(R0=exp(amak_std$value[which(amak_std$name=="log_Rzero")]), h=amak_output$Steep[2], phi=amak_output$phizero, sigmaR=om_input$logR_sd, mean2med=FALSE)$R0BC
  h=convertSRparms(R0=exp(amak_std$value[which(amak_std$name=="log_Rzero")]), h=amak_output$Steep[2], phi=amak_output$phizero, sigmaR=om_input$logR_sd, mean2med=FALSE)$hBC
  amak_msy_adhoc=msy_calcs_adhoc(steep=h, R0=R0, M=matrix(rep(om_input$M.age, times=om_input$nyr), nrow=om_input$nyr, byrow=T), wgt=om_input$W.mt, prop.f=om_input$proportion.female, selL=amak_output$sel_fsh_1[1,3:ncol(amak_output$sel_fsh_1)], selD=rep(0,om_input$nages), selZ=amak_output$sel_fsh_1[1,3:ncol(amak_output$sel_fsh_1)], mat.f=om_input$mat.age, mat.m=NULL, BC=1, maxF=1.0, step=0.001)
...
for(om_sim in 1:om_sim_num){
  ...
  R0=convertSRparms(R0=asap_output$SR.parms$SR.R0*1000, h=asap_output$SR.parms$SR.steepness, phi=asap_output$SR.parms$SR.SPR0, sigmaR=om_input$logR_sd, mean2med=FALSE)$R0BC
  h=convertSRparms(R0=exp(amak_std$value[which(amak_std$name=="log_Rzero")]), h=asap_output$SR.parms$SR.steepness, phi=amak_output$phizero, sigmaR=om_input$logR_sd, mean2med=FALSE)$hBC
  asap_msy_adhoc <- msy_calcs_adhoc(steep=h, R0=R0, M=matrix(rep(om_input$M.age, times=om_input$nyr), nrow=om_input$nyr, byrow=T), wgt=om_input$W.mt, prop.f=om_input$proportion.female, selL=asap_output$fleet.sel.mats$sel.m.fleet1[1,], selD=rep(0,om_input$nages), selZ=asap_output$fleet.sel.mats$sel.m.fleet1[1,], mat.f=om_input$mat.age, mat.m=NULL, BC=1, maxF=1.0, step=0.001)
...
jpeg(file=file.path(maindir, subdir, "Rzero_BH_sim.jpg"), width=150, height=120, units="mm", res=300)
...
col= c("orange", "green", "red", "deepskyblue3")
jpeg(file=file.path(maindir, subdir, "Rzero_MSY_sim.jpg"), width=150, height=120, units="mm", res=300)
temp <- matrix(NA, nrow=om_sim_num, ncol=5)
for(k in 1:5){
  temp[,k] <- parameter_val_list[[k]][,1]
}
...
for(k in 1:5){
  for (om_sim in 1:om_sim_num){
    ...
    if(k==2){
      setwd(file.path(maindir,"output", "AMAK", paste("s", keep_sim_id[om_sim], sep="")))
      amak_std <- readRep("amak", suffix = ".std")
      temp$Rzero[om_sim] <- exp(amak_std$value[which(amak_std$name=="log_Rzero")])/1000*exp(0.18)
      temp$Q[om_sim] <- exp(amak_std$value[which(amak_std$name=="log_q_ind[1]")])
    }
    if(k==3){
      asap_output <- dget(file.path(maindir, "output", "ASAP", paste("s", keep_sim_id[om_sim], sep=""), "asap3.rdat"))
      temp$Rzero[om_sim] <- asap_output$SR.parms$SR.R0*exp(0.18)
      temp$Q[om_sim] <- asap_output$q.indices/1000
    }
    if(k==4){
      bam_output <- dget(file.path(maindir, "output", "BAM", paste("s", keep_sim_id[om_sim], sep=""), "bam-sim.rdat"))
...
col= c("orange", "green", "red", "deepskyblue3")
jpeg(file=file.path(maindir, subdir, "Rzero_MSY_Adhoc_sim.jpg"), width=150, height=120, units="mm", res=300)
temp <- matrix(NA, nrow=om_sim_num, ncol=5)
for(k in 1:5){
  temp[,k] <- parameter_val_list[[k]][,1]
}
