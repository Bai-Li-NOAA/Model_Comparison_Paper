...
popsim<-function(x){
  ...
  for (i in 1:(nyr-1)){
    ...
    N.age[(i+1),1]=BH.fcn(S=SSB[i],h=h,R0=R0,Phi.0=Phi.0)*exp(logR.resid[i+1])*BC
    for (a in 1:(nages-1))
  ...
  msy=msy_calcs(steep=h, R0=R0, M=M.age, wgt=W.mt, prop.f=prop.f, selL=selex_fleet$fleet1, selD=selex.D, selZ=selex_fleet$fleet1, mat.f=mat.age, mat.m=NULL, BC=BC, maxF=1.0, step=0.001)
