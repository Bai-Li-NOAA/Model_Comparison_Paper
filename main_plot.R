...
subdir = "SS"
for(om_sim in 1:om_sim_num){
  ss_output <- SS_output(dir=file.path(maindir, subdir, paste("s", keep_sim_id[om_sim], sep="")), ncols = 300, verbose=F, printstats=F)
  ...
  ss_agecomp[[om_sim]] <- apply(ss_output$natage_annual_2_with_fishery[1:par.sim1$nyr, 5:ncol(ss_output$natage_annual_2_with_fishery)], 1, function(x) x/sum(x))
}
...
save(om_list, amak_list, asap_list, bam_list, ss_list, file=file.path(maindir, "Outputs", "om_em_output.RData"))
...
for (j in figure_id){
  ...
  for(i in 1:length(plot_data_id)){
    ...
    lines(sim1$yr, ss_list[[plot_data_id[i]]][,j], typ="l", col=col[5], lwd=1.5)
...
for (j in figure_id){
  ...
  for(i in 1:length(plot_data_id)){
    ...
    lines(sim1$yr, ss_list[[plot_data_id[i]]][,j], typ="l", col=col[5], lwd=1)
...
for(j in figure_id){
  ...
  agecom_last[,5] <- as.matrix(ss_list[[plot_data_id]][[j]][, sim1$yr[length(sim1$yr)]])
  ...
  legend("topright", c("OM", "AMAK", "ASAP", "BAM", "SS"), pch=c(19, NA, NA, NA, NA), lty=c(NA,1,1,1,1), lwd=c(NA, 1.5, 1.5, 1.5, 1,5), col=col, bty="n", cex=0.8, title=paste("Year", sim1$yr[length(sim1$yr)]))
  dev.off()
}
...
for (i in 1:length(plot_data_id)){
  ...
  boxplot(amak_list[[plot_data_id[i]]][par.sim1$nyr,], asap_list[[plot_data_id[i]]][par.sim1$nyr,], bam_list[[plot_data_id[i]]][par.sim1$nyr,], ss_list[[plot_data_id[i]]][par.sim1$nyr,], col=col, axes=F, ylab=rownames(plot_data)[i], pch=16)
  box()
  axis(1, at=1:4, labels = c("AMAK", "ASAP", "BAM", "SS"))
  axis(2)
...
#   ylim=range(om_list[[plot_data_id[i]+8]], unlist(temp_list))
#   for(k in 1:length(temp_list)){
#     plot(om_list[[plot_data_id[i]+8]][,1], type="l", col="blue", axes=F, xlab="", ylab=title[i], panel.first=grid(lty=1), ylim=ylim)
#     sapply(1:om_sim_num, function(x) lines(temp_list[[k]][,x], col="gray70"))
#     lines(om_list[[plot_data_id[i]+8]][,1], type="o", col="blue", pch=19, cex=0.5)
#     box()
#     axis(side=1, at=seq(5, par.sim1$nyr, by=5), labels=F)
