...
open_r_info_list("parms", false); 
	...
	wrt_r_item("q.survey1", mfexp(log_q_survey1));  
        wrt_r_item("q.survey2", mfexp(log_q_survey2));     	       	    
...
open_r_df("parm.cons",1,8,2);
	...
	wrt_r_df_col("log_dm_survey1_ac",log_dm_survey1_ac_out);
        wrt_r_df_col("log_dm_survey2_ac",log_dm_survey2_ac_out);
    
	wrt_r_df_col("selpar_A50_survey1",selpar_A50_survey1_out);
    wrt_r_df_col("selpar_slope_survey1",selpar_slope_survey1_out);
        
        wrt_r_df_col("selpar_A50_survey2",selpar_A50_survey2_out);
    wrt_r_df_col("selpar_slope_survey2",selpar_slope_survey2_out);
    ...
    wrt_r_df_col("log_q_survey2",log_q_survey2_out);
...
open_r_vector("sdnr"); 
    ...
    wrt_r_item("sdnr.U.survey2", sdnr_I_survey2);
    wrt_r_item("sdnr.agec.survey1", sdnr_ac_survey1); 
    wrt_r_item("sdnr.agec.survey2", sdnr_ac_survey2); 
...
open_r_vector("like"); 
    ...
    wrt_r_item("lk.U.survey2", f_survey2_cpue);    
    wrt_r_item("lk.agec.survey1", f_survey1_agec);
    wrt_r_item("lk.agec.survey2", f_survey2_agec);	
    ...
    wrt_r_item("w.U.survey2", w_I_survey2);
    wrt_r_item("w.agec.survey1", w_ac_survey1);
    wrt_r_item("w.agec.survey2", w_ac_survey2);	
...
open_r_list("sel.age");
    ...
    open_r_matrix("sel.m.survey2");
    wrt_r_matrix(sel_survey2, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);   
    close_r_matrix();  
...
open_r_list("comp.mats");
    ...
    open_r_matrix("acomp.survey2.ob");		
    wrt_r_matrix(obs_survey2_agec, 2, 2);
    wrt_r_namevector(yrs_survey2_agec);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
    ...
    open_r_matrix("acomp.survey2.pr");		
    wrt_r_matrix(pred_survey2_agec, 2, 2);
    wrt_r_namevector(yrs_survey2_agec);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();  
...
open_r_df("t.series", styr, (endyr+1), 2);
    ...
    wrt_r_df_col("U.survey2.ob", obs_survey2_cpue_allyr);
    wrt_r_df_col("U.survey2.pr", pred_survey2_cpue_allyr);
    wrt_r_df_col("cv.U.survey2", survey2_cpue_cv_allyr);    
    ...
    wrt_r_df_col("acomp.survey2.n", nsamp_survey2_agec_allyr);   
    wrt_r_df_col("acomp.fleet1.n", nsamp_fleet1_agec_allyr);    
     
    wrt_r_df_col("acomp.survey1.nfish", nfish_survey1_agec_allyr); 
    wrt_r_df_col("acomp.survey2.nfish", nfish_survey2_agec_allyr); 
    ...
    wrt_r_df_col("acomp.survey2.neff",  w_ac_survey1*nsamp_survey2_agec_allyr);
    ...
    //wrt_r_df_col("acomp.survey2.neff", (1+nsamp_survey2_agec_allyr*exp(log_dm_survey2_ac_out(8)))/(1+exp(log_dm_survey2_ac_out(8))) );   
